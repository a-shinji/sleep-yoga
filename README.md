...
1. **Xcodeをインストール**（このMacには未導入）
   - Mac App Storeで「Xcode」を検索してインストール（無料・十数GB・時間がかかる）
   - 初回起動時に追加コンポーネントとiOSプラットフォームのインストールを求められたら実行
2. `NemuriYoga.xcodeproj` をXcodeで開く
3. 左のナビゲータでプロジェクト（NemuriYoga）→ TARGETS NemuriYoga → **Signing & Capabilities** タブ
   - **Team** に自分のApple IDを選択（未登録なら Xcode > Settings > Accounts で Apple ID を追加。無料でOK）
   - Bundle Identifier が他人と衝突するとエラーになるので、その場合は `dev.sasai.NemuriYoga` を適当に変える
4. iPhoneをUSBで接続し、画面上部のデバイス選択でそのiPhoneを選ぶ
...
| 声を変える | `make_guide.sh` の `VOICE`（`say -v '?'` で一覧）。品質を上げたいなら OpenAI TTS / ElevenLabs / VOICEVOX 等で生成したファイルを `NemuriYoga/Resources/guide.m4a` に差し替え |
| BGMを変える | `tools/make_bgm.py` を編集して再生成、または任意の音源を `NemuriYoga/Resources/bgm.m4a` に差し替え（ループ素材推奨） |
...
- プロジェクトファイルは [XcodeGen](https://github.com/yonaskolb/XcodeGen) で生成（`project.yml` が正、`xcodegen generate` で再生成）
...
- 「寝たまんまヨガ」は登録商標。公開時は名称に注意（本プロトタイプは無関係の名称にしてある）
